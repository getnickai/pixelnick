...
const LogoMark: React.FC<{ size: number; color?: string }> = ({
  size,
  color = BLUE,
}) => {
  // Unique clip id per instance so multiple marks in one doc don't collide.
  const clipId = `nickai_og_clip_${useId()}`;
  return (
  <svg
    width={size}
    height={size}
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: skin.bg, fontFamily: fontSans }}>
      ...
      >
        ...
        />
        {/* White NickAI mark centered on the wave panel. */}
        <div
          style={{
            position: "absolute",
            inset: 0,
