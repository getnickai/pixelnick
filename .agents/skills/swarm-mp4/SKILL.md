...
> ⚠️ Check the branch before running — the shared `~/Codex/pixelnick` checkout
...
- Handles come from `data/swarm-identity.ts` / the deck's `agents[].handle`
  (e.g. `GROK`, `GPT`, `Codex`, `GEMINI`, `KIMI`, `DEEPSEEK`, `QWEN`, `MISTRAL`).
...
cd "${PIXELNICK_DIR:-$HOME/Codex/pixelnick}"   # any up-to-date `main` checkout
