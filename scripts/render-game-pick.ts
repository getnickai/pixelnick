/**
 * Render the single-agent "Game Pick" card to PNG + MP4.
 *
 * Feeds each record from nick-upcoming.json (written by scripts/swarm-nick.ts)
 * straight through as `data` to the registered `game-pick-card` Remotion
 * composition → the React GamePickCardView. A record is a GamePickCardData
 * (fixture + Nick's single pick).
 *
 *   PNG = revealed final frame.   MP4 = the AI-analysis buffer → pick reveal.
 *
 * Flags (mirror render-results.ts):
 *   --game=<substr>   pick records whose `game` contains this (repeatable)
 *   --all             render every record
 *   --no-mp4          PNG only
 *   --feed=<path>     default public/swarm-arena-cards/nick-upcoming.json
...
const COMPOSITION_ID = "game-pick-card";
const PUBLIC_DIR = path.join(process.cwd(), "public");

type Rec = { home: string; away: string; game: string; pick?: { marketType: string; selection: string }; [k: string]: unknown };
...
function parse(argv: string[]) {
  ...
  const f = {
    all: false, mp4: true,
    feed: path.join(PUBLIC_DIR, "swarm-arena-cards", "nick-upcoming.json"),
...
function pick(records: Rec[], o: ReturnType<typeof parse>): Job[] {
  const chosen: Rec[] = o.all
    ...
    : o.games.flatMap((g) => {
        ...
        if (!matched.length) { console.warn(`  ! no upcoming record matches "${g}" — run swarm-nick.ts, or Nick has no pick on it.`); return []; }
        return [matched[0]];
      });
  return chosen.map((r) => ({
    slug: `game-pick-${slugify(r.home)}-vs-${slugify(r.away)}`,
    desc: `${r.home} vs ${r.away} [${r.pick?.marketType ?? "?"}/${r.pick?.selection ?? "?"}]`,
