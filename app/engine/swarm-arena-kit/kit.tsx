...
export function SwarmArenaKit() {
  ...
  // Defensive dedupe: the upstream feed has produced colliding handles before
  // (two agents reporting "GPT"); keep the first (higher-ranked) per handle so
  // selection and React keys stay sound. Rank = position in the deduped,
  // ROI-sorted deck.
    ...
    <div className="flex h-full min-h-0 w-full">
      ...
      <aside className="flex w-72 shrink-0 flex-col gap-5 overflow-y-auto border-r border-sidebar-border px-4 py-5">
        <div className="flex items-center justify-between px-1">
          ...
          <button
            onClick={refetch}
            disabled={deckState.status === "loading"}
            className="inline-flex items-center gap-1.5 rounded-md border border-input px-2 py-1 text-[11px] font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground disabled:opacity-50"
          >
            <RefreshCw
              className={cn("size-3", deckState.status === "loading" && "animate-spin")}
            />
            Refresh
          </button>
        </div>
        ...
        <p className="px-1 text-xs leading-relaxed text-muted-foreground">
          The Design card (Pixelnick Design → Static) rendered with the live R2
          deck — same component, real data.
        </p>
      ...
      >
        {deckState.status === "loading" ? (
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <button
              onClick={refetch}
              className="rounded-md border border-input px-3 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground"
            >
              Retry
            </button>
          </div>
